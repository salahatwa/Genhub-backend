...
public abstract class AbstractCrudService<DOMAIN, ID> implements CrudService<DOMAIN, ID> {
    ...
    public DOMAIN getById(ID id) {
        try {
			return fetchById(id).orElseThrow(() -> new Exception(domainName + " was not found or has been deleted"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
     ...
     */
    @Override
    public void mustExistById(ID id) throws Exception {
        if (!existsById(id)) {
            throw new Exception(domainName + " was not exist");
        }
    }
